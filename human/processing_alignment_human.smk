...
rule all:
    input:
        expand("{pdir}/human/2_alignments/{sample}_Aligned.out.unique.mbc.collapsed.{read}.sam",
            read=['fwd', 'rev'],
            pdir=DIRECTORY,
            sample=SAMPLE),
        expand("{pdir}/human/3_tss_data/raw_positions/{sample}_fwd.positions.csv",
            pdir=DIRECTORY,
            sample=SAMPLE),
        expand("{pdir}/combined/3_tss_data/raw_positions/all_mTECs_fwd.positions.csv",
            pdir=DIRECTORY,
            sample=SAMPLE),
        expand("{pdir}/human/2_alignments/{sample}_Aligned.out.unique.mbc.collapsed.fwd.{strand}.bam",
            strand=['plus', 'minus'],
            pdir=DIRECTORY,
            sample=SAMPLE),
        expand("{pdir}/human/2_alignments/{sample}_5prime_nz_readdepth.combined.bedgraph",
            pdir=DIRECTORY,
            sample=SAMPLE),
...
rule process_counts:
    ...
    output:
        bedgraph="{dir}/3_tss_data/bedgraphs/{sample}_fwd.bedgraph",
        counts="{dir}/3_tss_data/summary/{sample}_fwd.summary.counts.csv",
        positions="{dir}/3_tss_data/raw_positions/{sample}_fwd.positions.csv",
    shell:
        """
...
rule combine_counts:
    input:
        positions=expand("{{dir}}/human/3_tss_data/raw_positions/{sample}_fwd.positions.csv",
            sample=SAMPLE),
    output:
        positions="{dir}/combined/3_tss_data/raw_positions/all_mTECs_fwd.positions.csv",
    shell:
        """
        Rscript ~/analysis/tss/common-scripts/combine_counts.r \
            --indir {wildcards.dir}/human/3_tss_data/raw_positions \
