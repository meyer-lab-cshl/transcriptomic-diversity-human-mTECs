...
## list fo genes 
genes <- c("MLANA","CTRB2", "CYP3A4","USHBP1","STAR","LDHC","CELA2B","RARRES1","CHIT1","RHCG","CD46","TG")
genes <- c("APOH","CTRB2", "SAG")
chromosomes <- list("MLANA"="chr9","CTRB2" = "chr16", "CYP3A4"="chr7","USHBP1"="chr19","STAR"="chr8","LDHC"="chr11",
                    "CELA2B"="chr1","RARRES1"="chr3","CHIT1"="chr1","RHCG"="chr15","CD46"="chr1","TG"="chr8")
chromosomes <- list("APOH"="chr17","CTRB2" = "chr16", "SAG"="chr2")

entrezID <- list("APOH"= "350","CTRB2" ="440387", "SAG"="6295")
              
			  
starts <- list("MLANA"=5890802,"CTRB2" = 75237550, "CYP3A4"=99351650,"USHBP1"=17359350,"STAR"=37998880,"LDHC"=18430380,
               "CELA2B"=15800650,"RARRES1"=158412450,"CHIT1"=203184000,"RHCG"=90012230,"CD46"=207921900,"TG"=133862210)
ends <- list("MLANA"=5910606,"CTRB2" = 75241200, "CYP3A4"=99382900,"USHBP1"=17376000,"STAR"=38008780,"LDHC"=18473310,
             "CELA2B"=15819850,"RARRES1"=158451900,"CHIT1"=203199350,"RHCG"=90040450,"CD46"=207969400,"TG"=134150850)

starts <- list("APOH"=64206500,"CTRB2" = 75237570, "SAG"=234213500)
ends <- list("APOH"=64226000,"CTRB2" = 75241200, "SAG"=234256600)
...
for(i in genes){
  ...
  if(i==i){
    ...
    for(x in c("dense","squish")){
      ...
      plot_file_name = paste("/home/stroemic/hiwi_16/analysis/shifted_TSS/plots/gviz/new_",i,"_gviz_range_",x,"_oe.pdf",sep="")
    ...
    for(x in c("dense","squish")){
      ...
      plot_file_name = paste("/home/stroemic/hiwi_16/analysis/shifted_TSS/plots/gviz/new_",i,"_gviz_range_",x,".pdf",sep="")
      pdf(plot_file_name, width=12)
