...
main_TSSs_ranges <- function(x) {
  ...
  filt_ranges <- makeGRangesFromDataFrame(filt, keep.extra.columns = TRUE)
  o <- order(as.numeric(as.character(filt_ranges$geneID)))
  filt_ranges <- filt_ranges[o]
...
##### human 
###filtered
#raw positions, decide whether to use these or the filtered ones
##mtec all positions
...
#Main TSS +-10
main_tss_mTECs <- main_TSSs_ranges(mTEC_pos_fil)
main_tss10_mTECs <- main_tss_mTECs + 5
main_tss_tissues <- main_TSSs_ranges(tissues_pos_fil)
main_tss10_tissues <- main_tss_tissues + 5

#Main TSS +-100
main_tss100_mTECs <- main_tss_mTECs + 50
main_tss100_tissues <- main_tss_tissues + 50
...
for (j in 1) {
  ...
  main_TSS10_overlaps <- findOverlaps(mTEC_ranges, main_tss10_mTECs, select = "first")
  main_TSS100_overlaps <- findOverlaps(mTEC_ranges, main_tss100_mTECs, select = "first")
  ...
  counts_mTECs <- matrix(0, nrow=14, ncol=6) 
  rownames(counts_mTECs) <-c("main_TSS_10","main_TSS_100","TSS_10","TSS_100","first_exon","TSS_10_other_exons","TSS_100_other_exons","TTS_100","upstream","other_exon","intron","antisense","downstream","intergenic") 
  colnames(counts_mTECs) <-c("aire_dep_tras_mTECs", "fezf2_dep_tras_mTECs", "other_tras_mTECs", "hk_mTECs", "other_genes_mTECs","rest_mTECs")                
  
  start.time <- Sys.time()
  print("mTEC matrix")
  for (i in 1:length(mTEC_ranges)) { #length(mTEC_ranges)
    if (!is.na(main_TSS10_overlaps[i])) {
	  feature_class <- "main_TSS_10"
	  gene <- as.character(main_tss10_mTECs[main_TSS10_overlaps[i]]$geneID)
	
	} else if (!is.na(main_TSS100_overlaps[i])) {
	  feature_class <- "main_TSS_100"
      gene <- as.character(main_tss100_mTECs[main_TSS100_overlaps[i]]$geneID)
	
	} else if (!is.na(TSS10_overlaps[i])){
  ...
  saveRDS(counts_mTECs, file=paste("/home/stroemic/hiwi_16/analysis/gene_lists/raw_main_tss_new_classes_tss_oe_tra_san_aire_san_counts_mTECs_matrix_",class,".rds", sep=""))
  print("saved")
  ...
  ## findOverlaps and store in vectors
  main_TSS10_overlaps <- findOverlaps(tissue_ranges, main_tss10_tissues, select = "first")
  main_TSS100_overlaps <- findOverlaps(tissue_ranges, main_tss100_tissues, select = "first")
  ...
  ## define matrix to store counts calculated in loop
  counts_tissues <- matrix(0, nrow=14, ncol=6) ####change here
  rownames(counts_tissues) <-c("main_TSS_10","main_TSS_100","TSS_10","TSS_100","first_exon","TSS_10_other_exons","TSS_100_other_exons","TTS_100","upstream","other_exon","intron","antisense","downstream","intergenic") ##intron
  ...
  for (i in 1:length(tissue_ranges)) { 
    if (!is.na(main_TSS10_overlaps[i])) {
	  feature_class <- "main_TSS_10"
	  gene <- as.character(main_tss10_tissues[main_TSS10_overlaps[i]]$geneID)
	
	} else if (!is.na(main_TSS100_overlaps[i])) {
	  feature_class <- "main_TSS_100"
      gene <- as.character(main_tss100_tissues[main_TSS100_overlaps[i]]$geneID)
  ...
  saveRDS(counts_tissues, file=paste("/home/stroemic/hiwi_16/analysis/gene_lists/raw_main_tss_new_classes_tss_oe_tra_san_aire_san_counts_tissues_matrix_",class,".rds", sep=""))
  print("saved")

  #### further calculations and plotting ####
  counts <- cbind(counts_mTECs, counts_tissues)
  counts_trunc <- counts[-14,] ####change here and the colour vectors in plotting
  ...
  pdf(file =paste("/home/stroemic/hiwi_16/analysis/gene_lists/plots/raw_main_tss_new_classes_tss_oe_tra_san_aire_san_absolut_positions_proportions_by_matrix_",class,".pdf", sep=""))
  print(ggplot(m_counts, aes(x=gene_class,y=value,fill=feature_class)) + 
          ...
          scale_fill_manual(values = c('#6a3d9a','#ffff99','#a6cee3','#1f78b4','#fb9a99','#b2df8a','#33a02c','#e31a1c','#fdbf6f','#ff7f00','#cab2d6','#6a3d9a','#ffff99')) +
  ...
  pdf(file =paste("/home/stroemic/hiwi_16/analysis/gene_lists/plots/raw_main_tss_new_classes_tss_oe_tra_san_aire_san_absolut_positions_percentage_by_matrix_",class,".pdf",sep=""))
