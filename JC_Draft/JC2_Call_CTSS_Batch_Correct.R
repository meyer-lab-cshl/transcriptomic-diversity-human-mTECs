...
bam_files <- list.files("/grid/meyer/home/jacarter/TSS/5PSeq_Data", full.names=T, pattern="*.bam")
i=0
...
jpeg(file=paste(analysis_dir,'PowerLaw.jpeg',sep=''))
...
jpeg(file=paste(analysis_dir,'PowerLaw_Normalized.jpeg',sep=''))
plotReverseCumulatives(ce, values='normalized',fitInRange = c(5, 2000), onePlot = TRUE)
dev.off()
...
write.csv(df, paste(analysis_dir,"CAGEr_out/tagCount.csv",sep=''))
...
write.table(x[x$tagCounts>0,],paste(analysis_dir,"CAGEr_out/DFs/",subject,"_Counts_and_TPM",sep=""),row.names=FALSE,col.names=FALSE,sep="\t",,quote=FALSE)
write.table(x[x$tagCounts>0,c(features,"TPM")],paste(analysis_dir,"for_Paraclu/",subject,"_TPM_for_Paraclu.txt",sep=""),row.names=FALSE,col.names=FALSE,sep="\t",quote=FALSE)
}
