...
write.table(select(gtex_tra, Gene, ENSG),
...
tras_per_tissue <- expression_tra_tissues %>%
    pivot_longer(-c(Gene, ENSG, tau), names_to="tissue", values_to="status") %>%
    filter(status != 0) %>%
    group_by(tissue) %>%
...
p_all_tissues <- ggplot(tras_per_tissue) +
    geom_bar(aes(x=tissue, y=tras), stat='identity') +
    labs(x="GTEx tissues",
         y="Number of TRAs") +
    cowplot::theme_cowplot() +
...
p_tau_versus_tissue <-  ggplot(tissue_per_tra) +
    geom_boxplot(aes(x=as.factor(tissues), y=tau)) +
    labs(x="Number of GTEx tissues",
         y="Tau") +
    cowplot::theme_cowplot() +
    theme(axis.text = element_text(size=text_size),
