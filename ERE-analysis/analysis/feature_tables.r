...
generate_features <- function(anno_exons, anno_genes, anno_transcripts) {
    ...
    other_exon <- lapply(anno_exons, function(x) {
                        if( unique(strand(x)) == '+') x[-1] else x[-length(x)]
                      })
...
chr <- paste("chr", c(1:23, "M", "X", "Y"), sep="")
...
saveRDS(features, '~/TE_thymus/analysis/features.rds')
