...
saveRDS(list_peptides, file="/home/stroemic/hiwi_16/analysis/shifted_TSS/list_rest_of_peptides_protein_coding_genes.rds")


### make peptide dataframe to merge with gene list
peptides <- data.frame("rest_of_peptide"=unlist(list_peptides), "geneID" = names(list_peptides))
df_genes <- merge(df_genes, peptides, by="geneID")

df_genes$truncation_length <- 0
df_genes[!grepl("trunca",df_genes$rest_of_peptide),]$truncation_length <- nchar(as.character(df_genes[!grepl("trunca",df_genes$rest_of_peptide),]$rest_of_peptide))

top_genes <- df_genes[with(df_genes, order(p.value, -ratio, -truncation_length)),]
top_tras <- top_genes[top_genes$tra == "tra",]
write.table(top_genes, file="/home/stroemic/hiwi_16/analysis/shifted_TSS/sorted_protein_coding_genes_shifted_tss_plus_rest_of_peptide.csv", quote = FALSE, row.names = FALSE, col.names = TRUE, na = "", sep = ",")
write.table(top_tras, file="/home/stroemic/hiwi_16/analysis/shifted_TSS/sorted_tras_protein_coding_genes_shifted_tss_plus_rest_of_peptide.csv", quote = FALSE, row.names = FALSE, col.names = TRUE, na = "", sep = ",")
