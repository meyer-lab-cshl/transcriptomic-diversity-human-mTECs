...
for fwd, rev in itertools.izip( forward_file, reverse_file ):
	...
	#
	if ofiles_rev.has_key( rev.seq[0:options.sample_barcode] ):
		exact_match += 1
		if options.trim_molecular_barcode:
			...
			rev.write_to_fastq_file( ofiles_rev[ rev.seq[0:options.sample_barcode] ] )
		else:
	...
	#
		...
		# 
			...
			#one_mismatch += 1
				...
				#fwd[options.molecular_barcode:].write_to_fastq_file( ofiles[ bcMatched ] )
			#else:
				#fwd.write_to_fastq_file( ofiles[ bcMatched ] )
			# 
			## Write out information about the molecular barcode 
			# 
		...
		no_match += 1
		if options.trim_molecular_barcode:
			fwd[options.molecular_barcode:].write_to_fastq_file( ofiles_fwd[ ofiles_fwd.keys()[0] ] )
			rev.write_to_fastq_file( ofiles_rev[ ofiles_rev.keys()[0] ] )
		else:
			fwd.write_to_fastq_file( ofiles_fwd[ ofiles_fwd.keys()[0] ] )
			rev.write_to_fastq_file( ofiles_rev[ ofiles_rev.keys()[0] ] )
		# 
		## Write out information about the molecular barcode 
		# 
		#bcfiles[ rev.seq[0:options.sample_barcode] ].write( fwd.name + '\t' + fwd.seq[0:options.molecular_barcode] + '\n' )
		# Note: Added 22 March 2014 AFTER running the script in order to correct the output to molecular barcode file.
		bcfiles[ bcfiles.keys()[0] ].write( rfw_name + '\t' + fwd.seq[0:options.molecular_barcode] + '\n' )
