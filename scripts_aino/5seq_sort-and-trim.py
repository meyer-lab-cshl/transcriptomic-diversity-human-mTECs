...
for fwd, rev in itertools.izip( forward_file, reverse_file ):
	...
	#
	if ofiles.has_key( rev.seq[0:options.sample_barcode] ):
		exact_match += 1
		if options.trim_molecular_barcode:
			fwd[options.molecular_barcode:].write_to_fastq_file( ofiles[ rev.seq[0:options.sample_barcode] ] )
		else:
			fwd.write_to_fastq_file( ofiles[ rev.seq[0:options.sample_barcode] ] )
		# 
	...
	#
		...
		if matchWithMismatch == 1:
			one_mismatch += 1
			if options.trim_molecular_barcode:
				#fwd[molecular_barcode:].write_to_fastq_file( ofiles[ rev.seq[0:sample_barcode] ] )
				fwd[options.molecular_barcode:].write_to_fastq_file( ofiles[ bcMatched ] )
			else:
				#fwd.write_to_fastq_file( ofiles[ rev.seq[0:molecular_barcode] ] )
