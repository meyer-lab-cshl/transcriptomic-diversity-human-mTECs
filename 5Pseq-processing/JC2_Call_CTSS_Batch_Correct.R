...
pdf(file=paste(analysis_dir, 'PowerLaw.pdf', sep=''), width=4, height=4)
...
write.table(x[x$tagCounts>0,],
            paste(analysis_dir, "CAGEr_out/DFs/", subject, "_Counts_and_TPM",
                  sep=""),
            row.names=FALSE, col.names=FALSE, sep="\t", quote=FALSE)
write.table(x[x$tagCounts>0,c(features,"TPM")],
            paste(analysis_dir, "for_Paraclu/", subject, "_TPM_for_Paraclu.txt",
                  sep=""),
            row.names=FALSE, col.names=FALSE, sep="\t", quote=FALSE)
}
