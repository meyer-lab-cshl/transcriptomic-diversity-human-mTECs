...
for (j in 1:2) {
  ...
  counts_mTECs <- matrix(0, nrow=12, ncol=7) 
  ...
  colnames(counts_mTECs) <-c("aire_dep_tras_mTECs", "fezf2_dep_tras_mTECs", "other_tras_mTECs", "hk_mTECs", "mTEC_spec_mTECs", "thymus_spec_mTECs","others_mTECs")                
  ...
  for (i in 1:length(mTEC_ranges) ) { 
    ...
    if (identical(in_gene_classes, character(0))) in_gene_classes <- "others_mTECs"
  ...
  saveRDS(counts_mTECs, file=paste("/home/stroemic/hiwi_16/analysis/gene_lists/tss_oe_tra_san_aire_san_counts_mTECs_matrix_",class,".rds", sep=""))
  
  #### Analysis for tissue data ####
  ## make range objects for tags
  tissue_ranges <- makeGRangesFromDataFrame(features_dfs[[j]][[2]], keep.extra.columns = TRUE)
  
  gene_classes_tissues <- c("hk_tissues", "aire_dep_tras_tissues", "fezf2_dep_tras_tissues", "other_tras_tissues")    ####change here
  genes_entrez_tissues <- list(hk_genes$entrezgene, aire_dep_tras$entrezgene, fezf2_dep_tras$entrezgene, other_tras$entrezgene)    ####change here
  ...
  ## define matrix to store counts calculated in loop
  counts_tissues <- matrix(0, nrow=12, ncol=5) ####change here
  ...
  colnames(counts_tissues) <-c("hk_tissues", "aire_dep_tras_tissues", "fezf2_dep_tras_tissues", "other_tras_tissues", "others_tissues")   ####change here
  ...
  for (i in 1:length(tissue_ranges)) { 
    ...
    if (identical(in_gene_classes, character(0))) in_gene_classes <- "others_tissues"
  ...
  saveRDS(counts_tissues, file=paste("/home/stroemic/hiwi_16/analysis/gene_lists/tss_oe_tra_san_aire_san_counts_tissues_matrix_",class,".rds", sep=""))
  
  #### further calculations and plotting ####
  counts <- cbind(counts_mTECs, counts_tissues)
  counts_trunc <- counts[-12,c(1:6,8:11)] ####change here
  ...
  pdf(file =paste("/home/stroemic/hiwi_16/analysis/gene_lists/plots/tss_oe_tra_san_aire_san_absolut_positions_proportions_by_matrix_",class,".pdf", sep=""))
  ...
  pdf(file =paste("/home/stroemic/hiwi_16/analysis/gene_lists/plots/tss_oe_tra_san_aire_san_absolut_positions_percentage_by_matrix_",class,".pdf",sep=""))
  ...
  for (i in 1:11) {
    ...
    plot_df <- data.frame("feature"=counts_trunc[i,], 
                          "source"=factor(names(counts_trunc[i,]), levels = c("aire_dep_tras_mTECs", "fezf2_dep_tras_mTECs", "other_tras_mTECs", "hk_mTECs", "mTEC_spec_mTECs", "thymus_spec_mTECs", "hk_tissues", "aire_dep_tras_tissues", "fezf2_dep_tras_tissues", "other_tras_tissues")))
