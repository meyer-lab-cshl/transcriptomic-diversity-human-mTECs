...
for(i in files){
  ...
  dat$name <- paste(dat$chromosome,":",dat$start,"..",dat$end,",",dat$strand, sep="")
  #normalise counts based on total read count
  dat <- dat[with(dat, order(chromosome, start)),]
  ###export dataframe from dat
  dat_export <- data.frame("chrom"=dat$chromosome, "start"=dat$start, "end"=dat$end,"name"=dat$name, "dataValue"=dat$count, "strand"=dat$strand)
  dat_export <- dat_export[complete.cases(dat_export),]
  #export data frame as csv, do not use quotes, rownames; do use colnames
  write.table(dat_export, file =paste("/home/stroemic/hiwi_16/data/Summary_counts/bed/",name, ".bed", sep=""), row.names=FALSE, na="", col.names=FALSE, quote=FALSE, sep="\t")
  print(paste("/home/stroemic/hiwi_16/data/Summary_counts/bed/",name, ".bed", sep=""))
}
