...
for (i in c(250,1000,20000) ) {
  ...
  for (x in 1:nrow(mTEC_pos_fil)) {
    ...
    if (as.integer(rel_position) > i | as.integer(rel_position) < thres) {
      ...
      counts_mTECs[gene, rel_position] <- (counts_mTECs[gene, rel_position] + 1)
    }
  }
  end.time <- Sys.time()
  ...
  for (y in 1:nrow(tissues_pos_fil)) {
    ...
    if (as.integer(rel_position) > i | as.integer(rel_position) < thres) {
      ...
      counts_tissues[gene, rel_position] <- (counts_tissues[gene, rel_position] + 1)
    }
  }
  end.time <- Sys.time()
  time.taken <- end.time - start.time
...
saveRDS(list_uncounted_pos, file = "/home/stroemic/hiwi_16/analysis/gene_lists/tra_san_aire_san_position_list_uncounted_positions_human_filt.rds")
saveRDS(list_matrices, file="/home/stroemic/hiwi_16/analysis/gene_lists/tra_san_aire_san_position_list_counts_matrices_human_filt.rds")
saveRDS(list_counts_classes, file="/home/stroemic/hiwi_16/analysis/gene_lists/tra_san_aire_san_position_list_counts_classes_human_filt.rds")
...
for (i in c(250,1000,20000)) {
  ...
  # ggplot(m_counts_classes_tra_m, aes(x=Var2, y=value, group =Var1, colour=Var1 )) + 
  #   geom_point(size=0.5) +
  #   geom_line(aes(y=roll)) +
  #   labs(title="Distribution of positions over genomic range for different gene classes", x ="bp around annotated TSS", y ="", colour="")
  # 
  
  pdf(file=paste("/home/stroemic/hiwi_16/analysis/gene_lists/plots/tra_san_aire_san_gene_range_analysis_positions_",name,".pdf",sep=""))
  ...
  dev.off()
  pdf(file=paste("/home/stroemic/hiwi_16/analysis/gene_lists/plots/tra_san_aire_san_gene_range_analysis_positions_line_",name,".pdf",sep=""))
  ...
  dev.off()
  pdf(file=paste("/home/stroemic/hiwi_16/analysis/gene_lists/plots/tra_san_aire_san_gene_range_analysis_positions_line_only_",name,".pdf",sep=""))
...
################################################################
# mTEC_pos_fil_mut <- mTEC_pos_fil %>% group_by(geneID) %>% dplyr::select(rel_pos, BarcodeCount)
# 
# df_list <- split(mTEC_pos_fil[1:50000,], mTEC_pos_fil$geneID)
# 
# 
# 
# vect <- rep(0, 2001)
# names(vect) <- -1000:1000
# 
# Reduce("+", list)